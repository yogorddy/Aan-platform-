...
export default function CertificationTab({ onNavigateToAcademy }: CertificationTabProps) {
  ...
  return (
    <div className="space-y-6 text-slate-600 font-sans text-left">
      
      {/* Visual Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-5">
        <div className="space-y-1">
          <span className="text-[10px] font-mono uppercase tracking-wider text-blue-600 font-extrabold bg-blue-50 px-2.5 py-0.5 border border-blue-250 rounded">
            Protocol Certifications
          </span>
          <h2 className="text-xl font-mono tracking-tight font-extrabold text-slate-900 flex items-center gap-2">
        ...
        {isAcademyEnabled() && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 hover:text-blue-700 border border-blue-200 text-[10px] uppercase font-bold font-mono transition-all cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200/80 p-6 rounded-xl space-y-6 shadow-sm">
          
          <div className="space-y-1 text-left">
            <h3 className="font-mono text-xs font-black uppercase text-blue-600 tracking-wider">
          ...
          {checking ? (
            ...
            <div className="space-y-4 text-left font-mono">
              <div className="space-y-2 max-h-72 overflow-y-auto scrollbar-thin">
                {checkResults.map((res) => (
                  <div key={res.id} className="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-start gap-2.5">
                    {res.verdict ? (
                    ...
                    <div>
                      <span className="text-xs text-slate-900 font-bold block">{res.name}</span>
              ...
              {certified && (
                <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-xl flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-emerald-100 border border-emerald-300 flex items-center justify-center text-emerald-600 shrink-0 shadow-sm">
                    <Award className="w-6 h-6 animate-spin-slow text-yellow-500" />
                  </div>
                  <div>
                    <h4 className="text-xs font-black uppercase tracking-wider text-emerald-700">Aan Integration Certified</h4>
                    ...
                    <span className="text-[9px] text-slate-400 leading-none block font-mono mt-1.5 uppercase font-bold">
              ...
              <div className="flex gap-3 text-left pt-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-700 font-mono text-xs rounded-lg cursor-pointer"
                >
                  Clear Checks
                </button>
              </div>
            ...
            <div className="space-y-4 py-4 text-center max-w-md mx-auto">
              ...
              <div>
                <h4 className="text-xs font-mono font-bold text-slate-500">Prerequisites Ready for validation</h4>
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/80 p-5 rounded-xl space-y-4 shadow-sm">
          
          <div className="pb-1 border-b border-slate-200 text-left">
            <h3 className="font-mono text-xs font-black uppercase text-slate-900 tracking-wider flex items-center gap-1.5">
              <Layers className="w-4 h-4 text-blue-600" />
              2. Migration Assistant Panel
            </h3>
            <p className="text-[10px] text-slate-500 font-sans">Guidelines for migrating seamlessly from legacy auth systems.</p>
          </div>

          <div className="flex overflow-x-auto gap-0.5 border-b border-slate-100 pb-1 scrollbar-none font-mono text-[9px] font-bold uppercase text-left">
            <button
              onClick={() => setActiveMigrationTab("plan")}
              className={`px-3 py-1.5 rounded transition-all cursor-pointer ${
                activeMigrationTab === "plan" ? "bg-blue-50 text-blue-600 border border-blue-200 font-semibold" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              Planning
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded transition-all cursor-pointer ${
                activeMigrationTab === "checklist" ? "bg-blue-50 text-blue-600 border border-blue-200 font-semibold" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              Checklist
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded transition-all cursor-pointer ${
                activeMigrationTab === "rollout" ? "bg-blue-50 text-blue-600 border border-blue-200 font-semibold" : "text-slate-500 hover:text-slate-800"
              }`}
            >
              Rollout Guide
            </button>
          ...
          {activeMigrationTab === "plan" && (
            <div className="space-y-3.5 text-xs text-left leading-relaxed animate-fadeIn">
              <p className="text-slate-500 text-[11px]">
                Aan (Antigravity Assurance Network) operates as a decentralized, privacy-preserving trust standard layer. It doesn't replace legacy database directories; it augments them to block duplicated robots.
              </p>
              
              <div className="space-y-2 font-mono text-[10px]">
                <div className="p-2.5 bg-slate-50 rounded border border-slate-200">
                  <span className="text-blue-600 font-extrabold block">Phase 1: Silent Discovery</span>
                  <p className="text-slate-500 mt-0.5">Integrate <code className="text-slate-500 font-bold">monitor_only</code> policies to silently evaluate registration patterns without blocking standard signup loops.</p>
                </div>
                <div className="p-2.5 bg-slate-50 rounded border border-slate-200">
                  <span className="text-blue-600 font-extrabold block">Phase 2: Shadow Verification</span>
          ...
          {activeMigrationTab === "rollout" && (
            <div className="space-y-3 text-xs text-left leading-relaxed animate-fadeIn">
              ...
              <p className="text-slate-500 text-[11px]">
                To guarantee zero disruption for active customers, we recommend a <b className="text-slate-900">canary rollout schedule</b>:
              </p>
              
              <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 font-mono text-[10px] space-y-1.5 text-slate-500">
