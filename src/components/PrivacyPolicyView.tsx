...
export default function PrivacyPolicyView({ onNavigate }: PrivacyPolicyViewProps) {
  ...
  return (
    <div className="relative min-h-screen bg-[#050507] text-[#8c919d] selection:bg-emerald-500/10 selection:text-white print:bg-white print:text-black">
      
      {/* 1. SCROLL PROGRESS INDICATOR */}
      <div className="fixed top-0 left-0 right-0 h-1 bg-[#121318] z-50 print:hidden">
        <div 
          className="h-full bg-emerald-400 transition-all duration-75 shadow-[0_0_8px_rgba(16,185,129,0.5)]" 
          style={{ width: `${scrollProgress}%` }}
        />
      </div>

      {/* BACK BAR */}
      <div className="border-b border-white/[0.03] bg-[#08090c]/80 backdrop-blur px-6 py-4 sticky top-1 z-40 print:hidden">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <button 
            onClick={() => onNavigate('landing')}
            className="flex items-center gap-2 text-xs text-[#646e7a] hover:text-white transition-all font-mono font-semibold uppercase tracking-wider group cursor-pointer"
          >
      ...
      {/* 2. HERO SECTION */}
      <header className="py-24 px-6 md:py-32 border-b border-white/[0.03] bg-[#050507] print:py-10 relative">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-12 gap-12" ref={docContainerRef}>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left font-sans text-xs py-2 px-3 rounded transition-all flex items-center gap-2.5 cursor-pointer ${
                      isActive 
                        ? 'bg-white/[0.03] text-white font-semibold border-l-2 border-emerald-400' 
        ...
        <main className="col-span-1 lg:col-span-9 max-w-[800px] mx-auto w-full print:max-w-none print:p-0">
          ...
          {/* SEARCH BAR */}
          <div className="mb-12 flex flex-col sm:flex-row items-center gap-4 bg-white/[0.01] p-4 rounded-xl border border-white/[0.04] print:hidden">
            <div className="relative w-full flex-1">
              ...
              {searchTerm && (
                <button 
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-mono font-semibold text-slate-500 hover:text-white"
                >
                  CLEAR
                </button>
              )}
            ...
            <div className="flex items-center gap-2 shrink-0 select-none">
              <button 
                ...
                className="text-[11px] font-mono font-semibold text-slate-400 hover:text-white bg-white/[0.02] hover:bg-white/[0.04] px-3 py-2 rounded-md border border-white/[0.06] transition-all cursor-pointer"
              >
                EXPAND ALL
              </button>
              <button 
            ...
            {filteredSections.map((sec) => {
              ...
              return (
                <div 
                  ...
                  className="group pt-12 border-t border-white/[0.03] first:border-0 first:pt-0 print:border-slate-300 print:pt-6 animate-fade-in"
                >
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div className="flex items-center gap-2.5 font-mono text-[10px] text-emerald-400 font-semibold tracking-widest uppercase">
                    ...
                    <div className="flex items-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity print:hidden">
                      <button 
                        ...
                        className="flex items-center gap-1.5 text-[10px] font-mono font-semibold bg-white/[0.02] text-slate-400 hover:text-white px-2.5 py-1 rounded border border-white/[0.06] cursor-pointer"
                      >
                  ...
                  >
                    <div className="max-w-[700px]">
                      <h3 className="text-2xl sm:text-3xl font-sans font-light text-white tracking-tight leading-tight group-hover/hdr:text-emerald-400 transition-colors print:text-black">
                        {highlightText(`${sec.num}. ${sec.title}`, searchTerm)}
                      </h3>
                      <p className="text-sm text-slate-400 mt-2 font-sans font-light leading-relaxed print:text-black">
                  ...
                  <AnimatePresence initial={false}>
                    {isExpanded && (
                      ...
                      >
                        <div className="pt-6 pb-2 space-y-5 text-sm sm:text-base text-slate-400 leading-relaxed font-sans font-light max-w-[760px] print:text-black">
